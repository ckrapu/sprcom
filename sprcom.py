...
def spatial_community_regression(X,Y,C,W,setting = 'mvcar',response='bernoulli',poisson_base='none',
        ...
        community_effect_setting : string
            Type of model to implement. Choices are 'mvcar', 'car', 'nonspatial'
            and 'none'. 'nonspatial' assumes that each community gets a per-
            site random effect. 'mvcar' implements the linear model of
            coregionalization to obtain correlated spatial processes underlying
            the community scores. 'car' treats these spatial processes as
            independent. 'none' removes them entirely.
        response : str
            Distribution appropriate for the response contained in <Y>.
            Currently supported options are 'bernoulli' for binary data and
            'poisson' or 'nb' (negative binomial) for count-valued data
        ...
        response_fixed : bool
            ...
            values.
        save_mu : bool
            Determines whether or not the linear predictor mu is saved as a
            variable in PyMC3.
        phi_prior : string
            Choice of prior to use for entries of phi. Current supported options
            are 'half-normal' and 'exponential'.
        phi_scale : float
            Scale hyperparameter for phi.
        A_scale_param: float
            Parameter controlling the Half-Cauchy prior on the diagonal
            elements of the coregionalization matrix.
        per_site_effect : bool
            Determines whether or not a per-site random effect is added.
        Returns
        -------
        ...
        with pm.Model() as model:
            ...
            if setting.lower() == 'nonspatial':
                pe_var           = pm.InverseGamma('pe_var', alpha = 0.1, beta = 0.1, shape = C)
                community_effect = pm.Normal('community_effect',shape=[N,C],sd=pe_var**0.5)
            ...
            if per_response_intercept:
                ...
                if response_fixed:
                    response_effect = pm.Deterministic('response_effect',tt.as_tensor_variable(re_testval))
                else:
            ...
            if phi_prior == 'half-normal':
                phi = pm.HalfNormal('phi', sd=1/phi_scale,shape=[C,S])
            elif phi_prior=='exponential':
                phi = pm.Exponential('phi',lam=1/phi_scale,shape=[C,S])


            mu  = pm.math.dot(theta, phi) + intercept + response_effect
            if per_site_effect:
                se_var      = pm.InverseGamma('se_var',alpha=0.1,beta=0.1)
                beta_se     = pm.Normal('beta_se',sd=se_var**0.5,shape=[P,1])
                site_effect = pm.Deterministic('site_effect',pm.math.dot(X,beta_se))
                mu += site_effect
            ...
            elif response.lower() == 'nb':
                inv_alpha = pm.Exponential('inv_alpha',lam=5,shape = S)
                response = pm.NegativeBinomial('response',mu=pm.math.exp(mu),alpha=inv_alpha**-1,observed=Y)
        return model
